...
/* Include libraries required specifically by this implementation of this library and not already included by this library's header*/
...
int parsePayload(unsigned char *payload, unsigned char pLength) {
	...
	while (bytesParsed < pLength)
	{
		...
		"length" bytes of data from the payload as appropriate for your application.*/
		/*待办事项：基于扩展的codelevel、代码、长度和代码定义表，根据您的应用程序，处理来自有效负载的下一个“长度”字节数据。*/
...
int main(int argc, char **argv) {
	...
	unsigned char c;
	unsigned char i;
	/* TODO: Initialize 'stream' here to read from a serial data stream, or whatever stream source is appropriate
	for your application. See documentation for "Serial I/O" for your platform for details.*/
